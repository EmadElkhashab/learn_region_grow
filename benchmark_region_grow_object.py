...
def pick_seeds_per_object(obj_id, curvatures):
    """One seed per GT object: lowest-curvature point inside each obj_id.

    Returns a list of seed point indices sorted by ascending curvature so
    sequential `visited` semantics match the curvature-first ordering used
    by benchmark_region_grow_class.py.
    """
    seed_per_obj = []
    for oid in np.unique(obj_id):
        candidates = np.where(obj_id == oid)[0]
        seed_id = candidates[np.argmin(curvatures[candidates])]
        seed_per_obj.append((curvatures[seed_id], int(seed_id)))
    seed_per_obj.sort()
    return [sid for _, sid in seed_per_obj]
...
def main():
    ...
    for area in args.areas:
        ...
        for room_id in range(len(all_points)):
            ...
            seed_order = pick_seeds_per_object(obj_id, curvatures)
